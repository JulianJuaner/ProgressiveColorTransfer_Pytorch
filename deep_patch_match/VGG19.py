...
class VGG19:
    ...
    def get_deconvoluted_feat(self, feat, curr_layer, init=None, lr=10, iters=13, display=False):
        ...
        for i, layer in enumerate(list(self.model)):
            if i >= start_layer and i <= end_layer:
                ...
                for p in l.parameters():
                    p.data = p.data.type(torch.cuda.FloatTensor)
                layers.append(l)
        net = nn.Sequential(*layers)

        noise = torch.nn.Parameter(init.type(torch.cuda.FloatTensor).clone().contiguous(), requires_grad=True)
        target = feat.type(torch.cuda.FloatTensor).detach()
        ...
        #noise = noise.weight.type(torch.cuda.FloatTensor)
